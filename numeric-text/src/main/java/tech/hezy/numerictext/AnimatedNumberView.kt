...
class AnimatedNumberView @JvmOverloads constructor(
    ...
    init {
        ...
        context.theme.obtainStyledAttributes(
            attrs,
            R.styleable.AnimatedNumberView,
            defStyleAttr,
            0
        ).apply {
            try {
                paint.textSize = getDimension(R.styleable.AnimatedNumberView_textSize, 40f)
                paint.color = getColor(R.styleable.AnimatedNumberView_textColor, 0xFF000000.toInt())
            } finally {
                recycle()
            }
        }
